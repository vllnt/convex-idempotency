...
`@vllnt/convex-idempotency` is a Convex component: Exactly-once idempotency key ledger for retried operations, as a Convex component It follows the vllnt Component
...
├── client/
│   ├── index.ts           # Idempotency client class (consumer-facing API)
